...
# For example, a Link's number of votes can be accessed by link.votes if "link" is a Link.
...
# QUIZ - make the function query() return the ID of the link that was submitted by user 62443 and has > 1000 votes.
def query():
...
# QUIZ - make the function query() return a list of the IDs of the links that were submitted by user 62443 sorted by submission time ascending.
def query():
...
# Or we can do the above with less code:
def query():
...
# QUIZ - implement the function build_link_index() that creates a python dictionary the maps a link's ID to the link itself
